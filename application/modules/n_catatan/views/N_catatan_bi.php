...
class N_catatan_bi extends CI_Controller
{
    ...
    public function simpan()
    {
        ...
        for ($i = 1; $i < $p['jumlah']; $i++) {
            ...
            if ($mode_form == "add") {
                $strq = "INSERT INTO t_catatan_bi (id_siswa,ta,catatan_mid,catatan_final) VALUES ('$id_siswa','$tasm','$catatan','$catatan_final')";
            } else if ($mode_form == "edit") {
                // Check if the student already has a record in t_catatan_bi
                $existingRecord = $this->db->query("SELECT id FROM t_catatan_bi WHERE ta = '" . $tasm . "' AND id_siswa = '" . $id_siswa . "'")->row();
                ...
                if (!$existingRecord) {
                    ...
                    $strq = 'UPDATE t_catatan_bi SET catatan_mid = "' . $catatan . '", catatan_final = "' . $catatan_final . '" WHERE ta = "' . $tasm . '" AND id_siswa = "' . $id_siswa . '"';
                }
            }
    ...
    public function index()
    {
        ...
        LEFT JOIN t_catatan_bi c ON a.id_siswa = c.id_siswa AND c.ta = '" . $this->d['tasm'] . "'
    ...
    public function import_nilai()
    {
        ...
        if ($file[$length - 1] == 'xlsx' || $file[$length - 1] == 'xls') {
            ...
            foreach ($data as $d) {
                ...
                $this->db->update('t_catatan_bi', $data1);
            }
