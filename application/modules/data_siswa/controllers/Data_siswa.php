...
class Data_siswa extends CI_Controller
{
    ...
    public function datatable()
    {
        ...
        foreach ($q_datanya as $d) {
            ...
            $data_ok[4] = '<a href="' . base_url() . $this->d['url'] . '/edit/' . $d['id'] . '" class="btn btn-xs btn-success"><i class="fa fa-edit"></i> Edit</a>
                ' . $link_aktif_user . '
    ...
    public function import_siswa()
    {
        ...
        if ($file[$length - 1] == 'xlsx' || $file[$length - 1] == 'xls') {
            ...
            for ($b = 2; $b < 500; $b++) {
                ...
                // Check if $tgl_lahir is a float (numeric value) before calling getValue()
                if (is_float($tgl_lahir)) {
                    // Handle the float value accordingly (for example, you can convert it to a date if needed)
                    // For instance, if $tgl_lahir represents an Excel date value (numeric representation), you can convert it like this:
                    $date_tgl_lahir = date($format = "Y-m-d", PHPExcel_Shared_Date::ExcelToPHP($tgl_lahir));
                } else {
                ...
                // Check if $tgl_lahir is a float (numeric value) before calling getValue()
                if (is_float($diterima_tgl)) {
                    // Handle the float value accordingly (for example, you can convert it to a date if needed)
                    // For instance, if $tgl_lahir represents an Excel date value (numeric representation), you can convert it like this:
                    $date_diterima_tgl = date($format = "Y-m-d", PHPExcel_Shared_Date::ExcelToPHP($diterima_tgl));
                } else {
                    // Handle other cases if necessary
                }
                ...
                if ($nis != "" || $nisn != "" || $nama != "") {
                    $data[] = "('$nis', '$nisn', '$nama', '$jk', '$tmp_lahir', '$date_tgl_lahir', '$agama', '$status', '$anakke', '$alamat', '$notelp', '$sek_asal', '$sek_asal_alamat', '$diterima_kelas', '$date_diterima_tgl', '$diterima_smt', '$ijazah_no', '$ijazah_thn', '$skhun_no', '$skhun_thn', '$ortu_ayah', '$ortu_ibu', '$ortu_alamat', '$ortu_notelp', '$ortu_ayah_pkj', '$ortu_ibu_pkj', '$wali', '$wali_alamat', '$notelp_rumah', '$wali_pkj')";
                    $no++;
                }
            }
