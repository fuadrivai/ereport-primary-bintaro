<div class="row">
    ...
    <div class="col-md-12">
        <div class="alert alert-warning" style="color: #000">
            ...
            <ul>
                ...
                <li>If the basic competencies have not been added yet, please click the <b><i>Add Topic</i></b> button. Each competency will be created as a gradable item in Buzz LMS under the <b>"E-report Score → Knowledge"</b> folder. To edit or delete a competency name, click the "<i class="fa fa-pencil"></i>" or "<i class="fa fa-remove"></i>" button respectively.</li>
                
                <li>To enter student scores, go to the <b>Agilix Buzz LMS gradebook</b> and input scores under each corresponding competency item. All scores should be entered on a <b><i>scale of 1–100</i></b>.</li>
                
                <li>If there are any updates or changes to scores in Buzz, please click the <b><i>Import Score from Buzz LMS</i></b> button to synchronize the latest scores into this system.</li>
            </ul>
        ...
        <div class="panel panel-default">
            <div class="panel-body">
                ...
                <?php if($detil_mp['kd_singkat']=="BTQ") {?>
                    ...
                    <!--</a>-->
                    <a href="<?php echo base_url(); ?>n_pengetahuan/import/<?php echo $detil_mp['id_mapel']."-".$detil_mp['id_kelas']; ?>" class="btn btn-danger">
                        <i class="fa fa-download"></i> Download Excel Template
                    </a>
                
                    <a href="<?php echo base_url(); ?>n_pengetahuan/upload/<?php echo $detil_mp['id_mapel']."-".$detil_mp['id_kelas']; ?>" class="btn btn-success">
    ...
    <div class="col-md-12">
        <div class="card">
            <div class="header">
                <h5 class="title"><?php if ($detil_mp['tingkat'] == '1' || $detil_mp['tingkat'] == '4'){ ?>Sumatif <?php }else{ ?> Knowledge <?php } ?> Score &raquo; <?php echo $detil_mp['nmmapel']." - ".$detil_mp['nmkelas']; ?></h5>
            </div>
            <div class="content">
                <p>
                    <a href="#" onclick="return edit(0);" class="btn btn-info"><i class="fa fa-plus-circle"></i> Create Topic</a>
                </p>
                <ul class="list-group" id="list_kd">
                    <div id="list_kd_2" style="margin-bottom: 10px"></div>
                    
                    <li class="list-group-item" onclick="return view_kd(<?php echo $detil_mp['id_mapel'].", ".$detil_mp['id_kelas']; ?>,'t');"><a href="#"><i class="fa fa-chevron-right"></i>  Midterm Score</a></li>
                    <li class="list-group-item" onclick="return view_kd(<?php echo $detil_mp['id_mapel'].", ".$detil_mp['id_kelas']; ?>,'a');"><a href="#"><i class="fa fa-chevron-right"></i>  Final Score</a></li>
                    <li class="list-group-item" onclick="return view_kd(<?php echo $detil_mp['id_mapel'].", ".$detil_mp['id_kelas']; ?>,'c');"><a href="#"><i class="fa fa-chevron-right"></i>  CATATAN DIKNAS</a></li>
                </ul>
            </div>
        </div>
    </div>
...
<script type="text/javascript">
    ...
    let isBtq = <?=$detil_mp['kd_singkat'] == "BTQ"?"true":"false";?>;
    ...
    function view_kd(id, kelas, jenis='h') {
        ...
        if (id == 0 && kelas == 0) {
            ...
            $.getJSON(base_url+"<?php echo $url; ?>/ambil_siswa/"+kelas+"/"+id+"/"+jenis, function(data) {
                ...
                if (jenis == 'c'){
                    html = '<table class="table table-condensed table-bordered table-hover"><thead><tr><th width="5%">No</th><th width="15%">Nama</th><th width="35%">Catatan Mid</th><th width="35%">Catatan Final</th></tr></thead><tbody>';
                    var i = 1;
                    ...
                    var i = 1;
                    $.each(data.data, function(k, v) {
                        html +=  `<tr><td>${i}</td><td>${v.nama}</td><td><input name="id_siswa[]" type="hidden" value="${v.ids}"><input ${isBtq?'':"disabled"} name="nilai[]" type="number" min="0" max="100" class="form-control input-sm" value="${v.nilai}"></td></tr>`;
                        i++;
                    });
...
function importFromBuzz() {
    ...
    $.ajax({
        type: "GET",
        url: base_url + "n_pengetahuan/import_from_agilix/<?php echo $detil_mp['id_mapel'] . '-' . $detil_mp['id_kelas']; ?>",
        beforeSend: function () {
...
function create_gradebook_items_in_buzz() {
    ...
    $.ajax({
        type: "GET",
        url: base_url + "n_pengetahuan/create_gradebook_items_in_buzz/<?php echo $detil_mp['id_mapel'] . '-' . $detil_mp['id_kelas']; ?>",
        beforeSend: function () {
