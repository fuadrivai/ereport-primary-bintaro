...
class Login extends CI_Controller {
    ...
    public function index() {
    	$this->d['p'] = "login";
        $this->load->view("v_login_modern", $this->d);
    }
    public function do_loginapi() {
    	$p = $this->input->get();
    	$uu 		= $this->security->xss_clean($p['username']);
    	$u = explode("-", $uu)[0];
    	$pp 	= $this->security->xss_clean($p['password']);
        $p = explode("-", $pp)[0];
        ...
        if($j_cek == 1) {
            ...
            // Add a redirect
            // Add a redirect
            header("Location: https://report.mhis.link/bintaro/primary/lihat_raport");
            exit;
        } else {	
			$d['status'] = "gagal";
