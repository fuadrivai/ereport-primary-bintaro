        ...
        if (id == 0 && kelas == 0) {
            ...
            $.getJSON(base_url+"<?php echo $url; ?>/ambil_siswa/"+kelas+"/"+id+"/"+jenis, function(data) {
                ...
                $.each(data.data, function(k, v) {
                    html += '<tr><td>'+i+'</td><td>'+v.nama+'</td><td><input name="id_siswa[]" type="hidden" value="'+v.ids+'"><textarea name="nilai_mid[]" class="form-control input-sm" value="'+v.nilai_mid+'" required>'+v.nilai_mid+'</textarea></td><td><textarea name="nilai[]" class="form-control input-sm" value="'+v.nilai+'" required>'+v.nilai+'</textarea></td></tr>';
                    i++;
                }); 
