...
class N_catatan_guru extends CI_Controller {
   ...
   public function import($bawa) {
        ...
        $kolom_uts = $kolom;
        $kolom_uas = ($kolom+1);
        $objPHPExcel->getActiveSheet()->setCellValue($this->kolom_xl[$kolom].'7', 'Catatan Mid');
        $kolom++;
        $objPHPExcel->getActiveSheet()->setCellValue($this->kolom_xl[$kolom].'7', 'Catatan Final');
        $kolom++;
    ...
    public function import_nilai() {
        ...
        if($file[$length -1] == 'xlsx' || $file[$length -1] == 'xls') {//jagain barangkali uploadnya selain file excel <span class="wp-smiley wp-emoji wp-emoji-smile" title=":-)">:-)</span>
            ...
            $arr_perdata = array();
            foreach ($data as $d) {
                $arr_perdata[] = "('".$d['tasm']."', '".$d['jenis']."', '".$d['id_guru_mapel']."', '".$d['id_mapel_kd']."', '".$d['id_siswa']."', '".$d['nilai']."', '".$d['nilai_mid']."')";
            }
    ...
    public function simpan_nilai() {
        ...
        foreach ($p['nilai'] as $s) {
            ...
            if ($cek > 0) {
                ...
                $this->db->query("UPDATE t_nilai_cat SET tasm = '".$this->d['tasm']."', nilai = '$s', nilai_mid = '".$p['nilai_mid'][$i]."' WHERE id_guru_mapel = '".$p['id_guru_mapel']."' AND id_mapel_kd = '".$p['id_mapel_kd']."' AND id_siswa = '".$p['id_siswa'][$i]."' AND jenis = '".$p['jenis']."'");
            } else {
                $this->db->query("INSERT INTO t_nilai_cat (tasm,jenis,id_guru_mapel, id_mapel_kd, id_siswa, nilai, nilai_mid) VALUES ('".$this->d['tasm']."', '".$p['jenis']."', '".$p['id_guru_mapel']."', '".$p['id_mapel_kd']."', '".$p['id_siswa'][$i]."', '".$s."', '".$p['nilai_mid'][$i]."')");
            }
            $i++;
        }
