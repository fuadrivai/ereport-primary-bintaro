...
class N_ekstra extends CI_Controller {
    ...
    public function simpan_nilai() {
        ...
        foreach ($p['nilai'] as $s) {
            ...
            //echo $this->db->last_query();
            //exit;
            $desk = $this->db->escape_str($p['nilai_d'][$i]);
            if ($cek > 0) {
                $jumlah_sudah ++;
                $this->db->query("UPDATE t_nilai_ekstra SET nilai = '$s', desk = '".$desk."' WHERE tasm = '".$this->d['tasm']."' AND id_ekstra = '".$p['id_ekstra']."' AND id_siswa = '".$p['id_siswa'][$i]."'");
            } else {
                $this->db->query("INSERT INTO t_nilai_ekstra (tasm, id_ekstra, id_siswa, nilai, desk) VALUES ('".$this->d['tasm']."', '".$p['id_ekstra']."', '".$p['id_siswa'][$i]."', '".$s."', '".$desk."')");
            }
            $i++;
        }
