...
class N_kelompok extends CI_Controller {
	function __construct() {
        parent::__construct();
        $this->sespre = $this->config->item('session_name_prefix');
        $this->d['admlevel'] = $this->session->userdata($this->sespre.'level');
        $this->d['admkonid'] = $this->session->userdata($this->sespre.'konid');
        $this->d['url'] = "n_kelompok";
        $this->d['idnya'] = "setkelompok";
        $this->d['nama_form'] = "f_setkelompok";
     ...
     public function cetak($bawa,$tasm=0) {
        ...
        $q_nilai_harian = $this->db->query("SELECT 
                                d.nama nmsiswa, a.id_kelompok_kd, a.id_siswa, a.catatan, if(a.jenis='h',CONCAT(a.nilai,'//',b.nama_kd),a.nilai) nilai
                                FROM t_nilai_kelompok a
                                LEFT JOIN t_kelompok_kd b ON a.id_kelompok_kd = b.id
                                ...
                                WHERE c.id_kelas = ".$pc_bawa[1]." AND b.id_kelompok = ".$pc_bawa[0]."
                                AND a.tasm = '".$tasm."'
        ...
        $q_nilai_ta = $this->db->query("SELECT 
                                ...
                                FROM t_nilai_kelompok a
                                LEFT JOIN t_guru_kelompok b ON a.id_guru_mapel = b.id 
                                LEFT JOIN t_kelas_siswa c ON CONCAT(a.id_siswa,LEFT(a.tasm,4)) = CONCAT(c.id_siswa,c.ta)
                                LEFT JOIN m_siswa d ON c.id_siswa = d.id
                                WHERE c.id_kelas = ".$pc_bawa[1]." AND b.id_kelompok = ".$pc_bawa[0]." AND a.jenis != 'h'
                                AND a.tasm = '".$this->d['tasm']."'
                                ORDER BY d.id")->result_array();

        $q_kd_guru_ini = $this->db->query("SELECT a.* 
                                    FROM t_kelompok_kd a
                                    LEFT JOIN m_kelas b ON a.tingkat = b.tingkat
                                    WHERE a.id_guru = '".$this->d['admkonid']."'
                                    AND a.id_kelompok = '".$pc_bawa[0]."'
                                    AND b.id = ".$pc_bawa[1]." 
        ...
        if (!empty($q_nilai_harian)) {
            foreach ($q_nilai_harian as $d) {
                ...
                $_desk = nilai_pre($detil_guru['kkm'],$pc_nilai_h[0],'eng');
                $do = do_lang($detil_guru['kkm'], $pc_nilai_h[0]);
                $idx1 = $d['id_siswa'];
                $idx2 = $d['id_kelompok_kd'];
                $catatan = $d['catatan'];
                $d_nilai[$idx1]['nama'] = $d['nmsiswa'];
                $d_nilai[$idx1]['h'][$idx2]['nilai_huruf'] = nilai_huruf($detil_guru['kkm'], $pc_nilai_h[0]);
                $d_nilai[$idx1]['h'][$idx2]['nilai_pre'] = nilai_pre($detil_guru['kkm'], $pc_nilai_h[0],'eng');
    ...
    public function import($bawa) {
        ...
        $pc_bawa = explode("-", $bawa);
        $detil_guru = $this->db->query("SELECT 
                                         a.id_guru, b.nama nmmapel, b.kkm, c.nama nmkelas, d.nama nmguru
                                         FROM t_guru_kelompok a
                                         INNER JOIN m_kelompok b ON a.id_kelompok = b.id 
                                         INNER JOIN m_kelas c ON a.id_kelas = c.id
                                         INNER JOIN m_guru d ON a.id_guru = d.id 
        ...
        $q_nilai_harian = $this->db->query("SELECT 
                                d.nama nmsiswa, a.jenis, a.id_kelompok_kd, a.id_siswa, if(a.jenis='h',CONCAT(a.nilai,'//',b.nama_kd),a.nilai) nilai
                                FROM t_nilai_kelompok a
                                LEFT JOIN t_kelompok_kd b ON a.id_kelompok_kd = b.id
                                LEFT JOIN t_kelas_siswa c ON CONCAT(a.id_siswa,LEFT(a.tasm,4)) = CONCAT(c.id_siswa,c.ta)
                                LEFT JOIN m_siswa d ON c.id_siswa = d.id
        ...
        if (!empty($d_nilai)) {
            ...
            foreach ($d_nilai as $ke => $dn) {
                ...
                $catatan_kelompok = str_replace('; ', '. ', implode("; ", $txt_desk)); 
    ...
    public function import_nilai() {
        ...
        $list_kd = $this->db->query("SELECT * FROM t_kelompok_kd 
                                    WHERE id_kelompok = '".$detil_mp['id_kelompok']."'
        ...
        if($file[$length -1] == 'xlsx' || $file[$length -1] == 'xls') {
            ...
            $x_id_kelompok = $_sheet->getCell('B4')->getCalculatedValue();
            $x_id_guru = $_sheet->getCell('B5')->getCalculatedValue();
            $x_id_kelas = $_sheet->getCell('B6')->getCalculatedValue();

            //echo $x_id_kelompok."/".$detil_mp['id_kelompok']."-".$x_id_guru."/".$detil_mp['id_guru']."-".$x_id_kelas."/".$detil_mp['id_kelas'];

            if ($x_id_kelompok != $detil_mp['id_kelompok'] || $x_id_guru != $detil_mp['id_guru'] || $x_id_kelas != $detil_mp['id_kelas']) {
            ...
            $id_kelompok = $detil_mp['id_kelompok'];
            ...
            for ($b = $idx_baris_mulai; $b < $idx_baris_selesai; $b++) {
                ...
                for ($k = $idx_kolom_mulai; $k < ($idx_kolom_hide); $k++) {
                    ...
                    $id_kelompok_kd = !empty($pc_hide[1]) ? $pc_hide[1] : 0;
                    
                    //echo $pc_hide[1];
                    /*
                    echo "Id_Siswa : ".$id_siswa.", ";
                    echo "NIlai : ".$nilai.", ";
                    echo "Hide : ".$id_kelompok_kd;
                    */

                    $data[] = array("tasm"=>$tasm, "jenis"=>"h", "id_guru_mapel"=>$id_guru_mapel, "id_kelompok_kd"=>$id_kelompok_kd, "id_siswa"=>$id_siswa, "nilai"=>$nilai, "catatan"=>'', "catatan_mid"=>'');
                    //echo "/";
                } 
                //echo "<br>";
            }
            ...
            $strq = "REPLACE INTO t_nilai_kelompok (tasm, jenis, id_guru_mapel, id_kelompok_kd, id_siswa, nilai, catatan, catatan_mid) VALUES ";
            $arr_perdata = array();
            foreach ($data as $d) {
                $arr_perdata[] = "('".$d['tasm']."', '".$d['jenis']."', '".$d['id_guru_mapel']."', '".$d['id_kelompok_kd']."', '".$d['id_siswa']."', '".$d['nilai']."', '".$d['catatan']."', '".$d['catatan_mid']."')";
            }
            ...
            redirect('n_kelompok/index/'.$id_guru_mapel);
    ...
    public function ambil_siswa($kelas) {
        ...
        if ($jenis == "h") {
            $ambil_nilai = $this->db->query("SELECT
                                        ...
                                        FROM m_siswa b 
                                        INNER JOIN t_nilai_kelompok a ON a.id_siswa = b.id
                                        INNER JOIN t_guru_kelompok c ON a.id_guru_mapel = c.id
                                        INNER JOIN t_kelompok_kd d ON a.id_kelompok_kd = d.id 
                                        INNER JOIN m_guru e ON c.id_guru = e.id 
                                        INNER JOIN m_kelompok f ON c.id_kelompok = f.id
                                        INNER JOIN t_kelas_siswa g ON a.id_siswa = g.id_siswa
                                        INNER JOIN m_kelas h ON g.id_kelas = h.id
                                        WHERE h.id = $kelas AND a.id_kelompok_kd = $id_kd
                                        AND a.jenis = '$jenis' 
                                        AND g.ta = '".$this->d['tahun']."'
                                        ORDER BY b.nama ASC")->result_array();
            //echo $this->db->last_query();
            //exit;
            
            $ambil_nilai = $this->db->query("SELECT 
                        m_siswa.id ids, m_siswa.nama, IFNULL(t_nilai_kelompok.nilai,0) AS nilai,
                        t_kelas_siswa.id_kelas
                        FROM m_siswa
                        LEFT JOIN t_nilai_kelompok ON m_siswa.id = t_nilai_kelompok.id_siswa
                        AND t_nilai_kelompok.jenis = 'h'
                        AND t_nilai_kelompok.id_guru_mapel = '$id_guru_mapel' 
                        AND t_nilai_kelompok.id_kelompok_kd = '$id_kd' 
                        LEFT JOIN t_kelas_siswa ON m_siswa.id = t_kelas_siswa.id_siswa
                        WHERE (t_nilai_kelompok.jenis = 'h' OR t_nilai_kelompok.jenis IS NULL)
                        AND t_kelas_siswa.ta = '".$this->d['tahun']."'
                        AND t_kelas_siswa.id_kelas = $kelas ORDER BY m_siswa.nama ASC")->result_array();
            ...
            $ambil_nilai = $this->db->query("SELECT
                                        ...
                                        INNER JOIN t_kelas_siswa g ON a.id_siswa = g.id_siswa
                                        INNER JOIN m_kelas h ON g.id_kelas = h.id
                                        WHERE h.id = $kelas AND c.id_kelompok = $id_kd
                                        AND a.jenis = '$jenis' 
                                        AND a.tasm = '".$this->d['tasm']."'
                                        ORDER BY b.nama ASC")->result_array();
            */
            ...
            m_siswa.id AS ids,
            m_siswa.nama,
            COALESCE(t_nilai_kelompok.catatan, '') AS nilai,
            COALESCE(t_nilai_kelompok.catatan_mid, '') AS nilai_mid,
            t_kelas_siswa.id_kelas
        FROM m_siswa
        LEFT JOIN t_nilai_kelompok ON m_siswa.id = t_nilai_kelompok.id_siswa
            AND t_nilai_kelompok.jenis = 'c'
            AND t_nilai_kelompok.id_guru_mapel = '$id_guru_mapel'
            AND t_kelas_siswa.ta = '".$this->d['tahun']."'
        LEFT JOIN t_kelas_siswa ON m_siswa.id = t_kelas_siswa.id_siswa
        WHERE (t_nilai_kelompok.jenis = 'c' OR t_nilai_kelompok.jenis IS NULL)
            AND t_kelas_siswa.id_kelas = $kelas ORDER BY m_siswa.nama ASC")->result_array();
            //j($ambil_nilai);
            //exit;
            //echo $this->db->last_query();
            ...
            */
            $ambil_nilai = $this->db->query("SELECT 
						t_nilai_kelompok.id_siswa ids, m_siswa.nama, IFNULL(t_nilai_kelompok.nilai,0) nilai
						FROM t_nilai_kelompok 
						INNER JOIN t_guru_kelompok ON t_nilai_kelompok.id_guru_mapel = t_guru_kelompok.id
						INNER JOIN m_siswa ON t_nilai_kelompok.id_siswa = m_siswa.id
						WHERE t_guru_kelompok.id = '$id_guru_mapel' AND t_nilai_kelompok.jenis = '$jenis' AND 
						t_nilai_kelompok.tasm = '".$this->d['tasm']."' ORDER BY m_siswa.nama ASC")->result_array();
            //j($ambil_nilai);
            //exit;
            //echo $this->db->last_query();
            //exit;
    ...
    public function simpan_nilai() {
        ...
        foreach ($p['nilai'] as $s) {
            if ($p['jenis'] == 'c'){
                $cek = $this->db->query("SELECT id FROM t_nilai_kelompok WHERE id_guru_mapel = '".$p['id_guru_mapel']."' AND id_kelompok_kd = '".$p['id_kelompok_kd']."' AND id_siswa = '".$p['id_siswa'][$i]."' AND jenis = '".$p['jenis']."' AND tasm = '".$this->d['tasm']."'")->num_rows();
                //$queri[] = $this->db->last_query();
                //exit;
                if ($cek > 0) {
                    $jumlah_sudah ++;
                    $this->db->query("UPDATE t_nilai_kelompok SET catatan = '$s', catatan_mid = '".$p['nilai_mid'][$i]."' WHERE id_guru_mapel = '".$p['id_guru_mapel']."' AND id_kelompok_kd = '".$p['id_kelompok_kd']."' AND id_siswa = '".$p['id_siswa'][$i]."' AND jenis = '".$p['jenis']."'");
                    //$queri[] = "update";
                } else {
                    $this->db->query("INSERT INTO t_nilai_kelompok (tasm,jenis, id_guru_mapel, id_kelompok_kd, id_siswa, catatan, catatan_mid) VALUES ('".$this->d['tasm']."', '".$p['jenis']."', '".$p['id_guru_mapel']."', '".$p['id_kelompok_kd']."', '".$p['id_siswa'][$i]."', '".$s."', '".$p['nilai_mid'][$i]."')");
                    //$queri[] = "add";
                }
            }else{
                $cek = $this->db->query("SELECT id FROM t_nilai_kelompok WHERE id_guru_mapel = '".$p['id_guru_mapel']."' AND id_kelompok_kd = '".$p['id_kelompok_kd']."' AND id_siswa = '".$p['id_siswa'][$i]."' AND jenis = '".$p['jenis']."' AND tasm = '".$this->d['tasm']."'")->num_rows();
                ...
                if ($cek > 0) {
                    $jumlah_sudah ++;
                    $this->db->query("UPDATE t_nilai_kelompok SET nilai = '$s' WHERE id_guru_mapel = '".$p['id_guru_mapel']."' AND id_kelompok_kd = '".$p['id_kelompok_kd']."' AND id_siswa = '".$p['id_siswa'][$i]."' AND jenis = '".$p['jenis']."'");
                    //$queri[] = "update";
                } else {
                    $this->db->query("INSERT INTO t_nilai_kelompok (tasm,jenis, id_guru_mapel, id_kelompok_kd, id_siswa, nilai) VALUES ('".$this->d['tasm']."', '".$p['jenis']."', '".$p['id_guru_mapel']."', '".$p['id_kelompok_kd']."', '".$p['id_siswa'][$i]."', '".$s."')");
                    //$queri[] = "add";
                }
            }
            $i++;
    ...
    public function hapus($id) {
        $this->db->query("DELETE FROM t_guru_kelompok WHERE id = '$id'");
        $d['status'] = "ok";
    ...
    public function index($id) {
        ...
        $this->d['list_kd'] = $this->db->query("SELECT * FROM t_kelompok_kd 
                                    WHERE id_guru = '".$this->d['detil_mp']['id_guru']."'
                                    AND id_kelompok = '".$this->d['detil_mp']['id_kelompok']."'
    ...
    public function list_kd($id) {
        $this->d['detil_mp'] = $this->db->query("SELECT 
                                        ...
                                        WHERE a.id  = '$id'")->row_array();
        $this->d['list_kd'] = $this->db->query("SELECT * FROM t_kelompok_kd 
                                    WHERE id_kelompok = '".$this->d['detil_mp']['id_kelompok']."'
                                    AND tingkat = '".$this->d['detil_mp']['tingkat']."'
                                    AND semester = '".$this->d['semester']."'
                                    AND jenis = 'P'")->result_array();
