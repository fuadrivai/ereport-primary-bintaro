...
class Set_kd extends CI_Controller {
    ...
    public function simpan() {
        ...
        if ($p['_mode'] == "add") {
            $this->db->query("INSERT INTO t_mapel_kd (id_guru, id_mapel, tingkat, no_kd, jenis, nama_kd, semester, mid_final) VALUES ('".$this->d['admkonid']."', '".$p['id_mapel']."', '".$p['tingkat']."', '".$p['kode']."', '".$p['jenis']."', '".addslashes($p['nama'])."', '".$this->d['semester']."', '".$p['semester']."')");

            $d['status'] = "ok";
            $d['data'] = "Data berhasil disimpan";
        } else if ($p['_mode'] == "edit") {
            $this->db->query("UPDATE t_mapel_kd SET id_mapel = '".$p['id_mapel']."', tingkat = '".$p['tingkat']."', no_kd = '".$p['kode']."', jenis = '".$p['jenis']."', mid_final = '".$p['semester']."', nama_kd = '".addslashes($p['nama'])."' WHERE id = '".$p['_id']."'");
